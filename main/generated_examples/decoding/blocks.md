...
CompletedProcess(args=['ffmpeg', '-y', '-hide_banner', '-loglevel', 'error', '-f', 'lavfi', '-i', 'testsrc2=size=1280x720:rate=30:duration=5', '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-g', '30', '-colorspace', 'bt709', '-color_primaries', 'bt709', '-color_trc', 'bt709', '/tmp/tmpg01nk6ir/video.mp4'], returncode=0)
```
...
```
raw.data.dtype = torch.int16, raw.data.shape = torch.Size([1, 4096]), raw.sample_rate = 44100
```
...
```
CompletedProcess(args=['ffmpeg', '-y', '-hide_banner', '-loglevel', 'error', '-i', '/tmp/tmpg01nk6ir/video.mp4', '-i', '/tmp/tmpg01nk6ir/audio.wav', '-c:v', 'copy', '-c:a', 'aac', '-shortest', '/tmp/tmpg01nk6ir/av.mp4'], returncode=0)
```
