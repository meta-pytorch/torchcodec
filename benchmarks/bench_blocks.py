...
def _make_blocks(path, device):
    ...
    return demuxer, PacketDecoder(demuxer, device=device), ColorConverter(device=device)


def _decode_sequential(path, device):
    demuxer, decoder, converter = _make_blocks(path, device)
    _consume(_convert(converter, _decode(decoder, _demux(demuxer))))


def _decode_prefetch_frames(path, device):
...
def _decode_prefetch_packets(path, device):
...
def _decode_prefetch_packets_and_frames(path, device):
...
def _decode_video_decoder(path, device):
...
def main():
    ...
    path = make_video()
    print(
        f"Video: {path}  ({_SOURCE} {_WIDTH}x{_HEIGHT} {_FPS}fps {_DURATION_S}s)  "
