...
def _repeat_last_policy(
    values: list[_INT_OR_FLOAT], desired_len: int
) -> list[_INT_OR_FLOAT]:
...
def _wrap_policy(values: list[_INT_OR_FLOAT], desired_len: int) -> list[_INT_OR_FLOAT]:
    # values = [1, 2, 3], desired_len = 5
    # output = [1, 2, 3, 1, 2]
...
def _error_policy(
    frames_indices: list[_INT_OR_FLOAT], desired_len: int
) -> list[_INT_OR_FLOAT]:
    raise ValueError(
