...
#include "EncodingCudaDeviceInterface.h"
...
namespace {
static bool g_encoding_cuda = registerEncodingDeviceInterface(
    EncodingDeviceInterfaceKey(torch::kCUDA),
    [](const torch::Device& device) {
      return new EncodingCudaDeviceInterface(device);
    });
...
EncodingCudaDeviceInterface::EncodingCudaDeviceInterface(
    const torch::Device& device)
    : EncodingDeviceInterface(device) {
  TORCH_CHECK(
      g_encoding_cuda, "EncodingCudaDeviceInterface was not registered!");
  TORCH_CHECK(
...
EncodingCudaDeviceInterface::~EncodingCudaDeviceInterface() {
  if (hardwareDeviceCtx_) {
    returnHardwareDeviceContextToCache(device_, std::move(hardwareDeviceCtx_));
  }
  returnNppStreamContextToCache(device_, std::move(nppCtx_));
}

void EncodingCudaDeviceInterface::initialize(
    [[maybe_unused]] AVCodecContext* codecContext) {
  TORCH_CHECK(codecContext != nullptr, "codecContext is null");
  // Any additional initialization can go here
}

std::optional<const AVCodec*> EncodingCudaDeviceInterface::findHardwareEncoder(
...
void EncodingCudaDeviceInterface::registerHardwareDeviceWithCodec(
...
void EncodingCudaDeviceInterface::setupHardwareFrameContext(
    AVCodecContext* codecContext) {
  TORCH_CHECK(codecContext != nullptr, "codecContext is null");
  TORCH_CHECK(
      hardwareDeviceCtx_, "Hardware device context has not been initialized");
...
UniqueAVFrame EncodingCudaDeviceInterface::convertTensorToAVFrame(
...
const Npp32f (*EncodingCudaDeviceInterface::getConversionMatrix(
    AVCodecContext* codecContext))[4] {
