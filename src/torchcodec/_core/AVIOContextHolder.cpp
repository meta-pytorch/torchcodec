...
void AVIOContextHolder::createAVIOContext(
  ...
  STABLE_CHECK(seek != nullptr, "seek method must be defined");

  if (isForWriting) {
    STABLE_CHECK(write != nullptr, "write method must be defined for writing");
  } else {
    STABLE_CHECK(read != nullptr, "read method must be defined for reading");
  }
  ...
  if (!avioContext_) {
    av_freep(&buffer);
    STABLE_CHECK(false, "Failed to allocate AVIOContext");
  }
}
