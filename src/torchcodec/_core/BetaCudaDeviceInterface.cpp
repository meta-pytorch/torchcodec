...
std::optional<cudaVideoChromaFormat> validateChromaSupport(
  ...
  // otherwise.
  STABLE_CHECK(desc != nullptr, "desc can't be null");
...
BetaCudaDeviceInterface::BetaCudaDeviceInterface(const StableDevice& device)
  ...
  STABLE_CHECK(g_cuda_beta, "BetaCudaDeviceInterface was not registered!");
  STABLE_CHECK(
...
void BetaCudaDeviceInterface::initialize(
  ...
  STABLE_CHECK(avStream != nullptr, "AVStream cannot be null");
  timeBase_ = avStream->time_base;
  frameRateAvgFromFFmpeg_ = avStream->r_frame_rate;

  const AVCodecParameters* codecPar = avStream->codecpar;
  STABLE_CHECK(codecPar != nullptr, "CodecParameters cannot be null");
...
void BetaCudaDeviceInterface::initializeBSF(
  ...
  STABLE_CHECK(codecPar != nullptr, "codecPar cannot be null");
  STABLE_CHECK(avFormatCtx != nullptr, "AVFormatContext cannot be null");
  STABLE_CHECK(
...
int BetaCudaDeviceInterface::streamPropertyChange(CUVIDEOFORMAT* videoFormat) {
  STABLE_CHECK(videoFormat != nullptr, "Invalid video format");
  ...
  if (!decoder_) {
    ...
    STABLE_CHECK(decoder_, "Failed to get or create decoder");
  }
...
int BetaCudaDeviceInterface::frameReadyForDecoding(CUVIDPICPARAMS* picParams) {
  STABLE_CHECK(picParams != nullptr, "Invalid picture parameters");
  STABLE_CHECK(decoder_, "Decoder not initialized before picture decode");
...
UniqueAVFrame BetaCudaDeviceInterface::convertCudaFrameToAVFrame(
  ...
  STABLE_CHECK(framePtr != 0, "Invalid CUDA frame pointer");
  ...
  STABLE_CHECK(width > 0 && height > 0, "Invalid frame dimensions");
  STABLE_CHECK(
      pitch >= static_cast<unsigned int>(width), "Pitch must be >= width");

  UniqueAVFrame avFrame(av_frame_alloc());
  STABLE_CHECK(avFrame.get() != nullptr, "Failed to allocate AVFrame");
...
UniqueAVFrame BetaCudaDeviceInterface::transferCpuFrameToGpuNV12(
  ...
  STABLE_CHECK(cpuFrame != nullptr, "CPU frame cannot be null");
  ...
  STABLE_CHECK(nv12CpuFrame != nullptr, "Failed to allocate NV12 CPU frame");
  ...
  STABLE_CHECK(gpuFrame != nullptr, "Failed to allocate GPU AVFrame");
