...
try:
    with expose_ffmpeg_dlls():
        ffmpeg_major_version, core_library_path, _pybind_ops = ensure_ffmpeg_loaded()
...
if _FFMPEG_AVAILABLE:
    ...
    get_frames_by_pts_in_range = (
        torch.ops.torchcodec_ns.get_frames_by_pts_in_range.default
    )
    get_frames_by_pts_in_range_audio = (
    ...
    def create_from_bytes(
        video_bytes: bytes, seek_mode: str | None = None
    ) -> torch.Tensor:
    ...
    def streaming_encoder_open_file_abstract(
        encoder: torch.Tensor, filename: str
    ) -> None:
        return
