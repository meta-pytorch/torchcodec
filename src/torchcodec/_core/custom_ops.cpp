...
// Get the metadata from the video as a string. Serialization lives in the
// torch-free core (MetadataJson) so the pybind frontend can reuse it.
std::string get_json_metadata(torch::stable::Tensor& decoder) {
  return getVideoJsonMetadata(unwrapTensorToGetDecoder(decoder));
}

// Get the container metadata as a string.
std::string get_container_json_metadata(torch::stable::Tensor& decoder) {
  return getContainerJsonMetadata(unwrapTensorToGetDecoder(decoder));
}
