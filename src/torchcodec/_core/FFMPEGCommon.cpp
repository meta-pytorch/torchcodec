...
AutoAVPacket::AutoAVPacket() : avPacket_(av_packet_alloc()) {
  STABLE_CHECK(avPacket_ != nullptr, "Couldn't allocate avPacket.");
}
...
const AVPixelFormat* getSupportedPixelFormats(const AVCodec& avCodec) {
  ...
  if (ret < 0 || supportedPixelFormats == nullptr) {
    STABLE_CHECK(false, "Couldn't get supported pixel formats from encoder.");
  }
#else
...
int getNumChannels(const AVCodecParameters* codecpar) {
  STABLE_CHECK(codecpar != nullptr, "codecpar is null");
#if LIBAVFILTER_VERSION_MAJOR > 8 || \
    (LIBAVFILTER_VERSION_MAJOR == 8 && LIBAVFILTER_VERSION_MINOR >= 44)
...
UniqueAVFrame allocateAVFrame(
  ...
  STABLE_CHECK(avFrame != nullptr, "Couldn't allocate AVFrame.");
...
SwrContext* createSwrContext(
  ...
  STABLE_CHECK(swrContext != nullptr, "Couldn't create swrContext");
...
UniqueSwsContext createSwsContext(
  ...
  SwsContext* swsContext = sws_getContext(
      ...
      nullptr,
      nullptr);
  STABLE_CHECK(swsContext, "sws_getContext() returned nullptr");
  ...
  int ret = sws_getColorspaceDetails(
      ...
      &contrast,
      &saturation);
  STABLE_CHECK(ret != -1, "sws_getColorspaceDetails returned -1");
  ...
  ret = sws_setColorspaceDetails(
      ...
      contrast,
      saturation);
  STABLE_CHECK(ret != -1, "sws_setColorspaceDetails returned -1");
